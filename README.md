---
doc: "dyad-milo — README"
kind: derived-view
genre: falsifiable-manifesto
rule: "cite the source, never this lens"
belief:
  statement: "A dyad is only as durable as the mind operating it. Without dyad-milo, the Operator won't have the mental-fitness to sustainably operate a dyad — even via wu-wei — over a lifetime."
...
> **This README is a lens, not a home.** It projects beliefs whose canonical home is
> `DYAD.md`. Cite the source, never this lens.

**A dyad is only as durable as the mind operating it.** Without dyad-milo, the Operator
won't have the mental-fitness to sustainably operate a dyad — even via wu-wei — over a
lifetime. That is a hypothesis, and every claim below hands you a way to break it.

## What it is

dyad-milo is a human+Agent dyad that builds a **mental-fitness system** — a durable, reusable
structure that builds the Operator's mental-fitness: **mental-capability** (intellectual *and*
emotional) plus the **mental-stamina** to sustain it, just as physical fitness is strength
plus endurance. It grounds on a concrete method — currently CBT (cognitive behavioral
therapy) — but the craft is the fitness system, not the method. The dyad doesn't deliver
therapy; it builds the system the Operator uses to build their own mental-fitness.
...
holds. The dyad's aim is to make itself unnecessary — success is the day the Operator sustains
the practice, and so sustains every other dyad, without needing dyad-milo in the loop.

## The ask

Attack the claims. Each is a hypothesis with an explicit way to break it; none has yet
survived an outside attack (coverage `E0`). Break one against dyad-milo's ledger as it
...
§ Craft_invariant, § Contract). This README only projects them — cite the source, never this
lens.
