...
4. **adversarial-validate (final step).** milo spawns an **adversarial sub-agent** — a separate context —
   ...
   **fail-closed** — a real break does not land, it surfaces for correction. The independent adversary is
   what keeps the autonomous land consistent with **no-self-ratify** (disposer ≠ generator). ADR-0007.
