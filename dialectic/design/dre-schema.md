...
essence fragment a non-empty `facet` + `text` and a valid `partition` if present; the body is non-empty
(its length is never checked). Filenames: `reflections/YYYY-MM-DD-NN.md`.
