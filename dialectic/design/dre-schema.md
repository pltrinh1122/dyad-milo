...
observations:                               # optional SHARED sub-class telemetry (§ 6 / ADR-0004)
  - programs: [<program-id>, ...]           #   which program(s) this datum feeds; ⊆ programs; omit = all
    at: <ISO instant>                       #   optional; open-extensible fields below
    intensity: <text> ; somatic: <text> ; situation: <text> ; antecedent: <text>
    thought: <text> ; behavior: <text> ; relief: <text> ; note: <text>
---
...
essence fragment a non-empty `facet` + `text` and a valid `partition` if present; `observations` (if
present) a list of mappings, each string field non-empty when given and open-extensible, each
